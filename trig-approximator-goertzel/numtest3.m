...
% wielkość bazy oraz skok -> numtest3(Nmax,jump)
...
fprintf("%70s\n", repmat('-',1,70));
fprintf("%s%s\n",repmat(' ',1,22),'f(x) = 2/pi * arcsin(sinx)');
fprintf("%70s\n\n", repmat('-',1,70));
...
legend show;
plot(x, y_exact, '-g','LineWidth', 2, 'DisplayName', 'y = 2/pi * arcsin(sinx)');
xlabel('x');
ylabel('f(x)');
